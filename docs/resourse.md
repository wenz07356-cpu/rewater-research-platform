...
#目标：
优化query环节，增强用户和后台的交互，根据docs\query_V5.md的要求，提供编写代码提示词。

#具体要求：
1.先不弄代码，分析现有代码，从后续编写代码的角度来提供给codex的提示词。
2.输出到docs\query_V6.md。

#目标：
我不打算继续优化参数，在现有评估体系下，根据app\rag_eval\ragas\runs\20260819T225042-fabf0091\run_results.csv的结果，优化评估问题集app\rag_eval\ragas\rag_gold_questions.csv，使得最终的结果看的过去，你先评估下是否可行，不做任何修改。


#目标：
对优化后的query环节，进行进一步修改。
#问题和优化方案：
1.首先前端资料范围这个，我觉得不需要。选文件、地域、文档政策、关键词、主题等这些可以跟前端的这些交互的没必要搞这么复杂，这部分可以取消。
2.四种模式是可以的：均衡、精确检索、全面检索、自定义。但是前端页面设计的不好，容易让用户产生歧义，点了精确搜索，下面的候选数默认值还是10，不仅还是10，还能修改，修改之后还不能复位，这个就给用户很大困扰，不知道到底是什么情况。应该正常情况下，点均衡、精确检索、全面检索的时候，下面的候选数这些不要出现，或者出现也应该是不能改变的默认值，前端点不动的。
3.有关联网搜索不管采用哪种模式，都能够让用户选择是否使用联网搜索，所以联网搜索的位置干脆放到流式输出按钮的前面去。不跟检索模式挂钩。
4.本会话记住选择这个按钮可以不要。
5.自定义模式下，只有当HyDE点开的时候，后面的HyDE才能让人选择。


#目标：
对优化后的query环节，进行进一步修改。
#问题和优化方案：
1.后端资料范围、会话记忆字段等还是得删除，后续要拿这个项目去面试，无关代码还是删除比较好，仅删除刚刚因资料范围、会话记忆字段而增加的无关代码，不能影响程序的正常执行，保障现有功能能够实现。当然，如果改动较大，可以不删除。
2.目前前端仍然存在，选择全面检索、均衡、精确检索的时候，下面的候选数默认值还是10，不仅还是10，还能修改，修改之后还不能复位，这个就给用户很大困扰，不知道到底是什么情况。应该正常情况下，点均衡、精确检索、全面检索的时候，下面的候选数这些不要出现，或者出现也应该是不能改变的默认值，前端点不动的。这个地方要修改掉，只能自定义的时候下面候选数那些才能修改，并且虽然全面检索、均衡、精确检索模式，下面候选数不能点动，但是默认值要正确。


#目标：
优化query前端页面，各个红色框的部分。
#具体要求：
1.最上面，掌柜智库知识库，改成：再生水政策标准知识库。
2."你好，我是掌柜智库知识库客服。你可以直接提问，我会在“阶段进度”里展示处理过程。" 这个最开始的语言，修改下，跟再生水相关的。
3."提示：可以问“如何使用万用表测量电压？”"，修改成跟再生水相关的。
4.左右两边头像，“我”和“掌柜智库”，也修改下，可以不用文字。换成两个动漫头像吧。掌柜智库用爱因斯坦。我用特斯拉，发明交流电的那个，用他们的动漫形象。
...
from app.shared.runtime.logger import logger, step_log等等，这些导包，在vs code里面是能够通过ctrl+确认点进去llm_provider、EMBEDDING_BATCH_SIZE这些，但是在pycharm里面，这些导包，在vs code里面是能够通过ctrl+确认点进去llm_provider、EMBEDDING_BATCH_SIZE这些，但是pycharm里面，这些导包点不进去。
但是在pycharm能够跑通。
#要求：
先找到原因，是项目根目录没配置好还是什么环境问题，需要重新配置，还是什么其他原因。然后提出解决方案。不做修改。

#确认：
1..idea里面旧项目是否可以删除，我想弄一个没有原来zhishiku的
2.纯 UI 操作，不用手改 XML你能直接操作吗？我给你权限。

#要求：先不修改也不删除，先告诉我答案。


我选择路线 A（删干净，最彻底）：关掉 PyCharm → 删掉整个 .idea 文件夹 → 重新打开项目。PyCharm 会用正确的名字 rewater-agent 全新生成一份配置，天然没有 zhishiku，app 会被正确识别。代价：丢失 workspace.xml 里的运行配置（我看到里面有 node_document_split.py 等 Run Configuration）和检查规则，需要重建。

这个重建会耗时吗？困难吗？重建后影响程序正常启动吗？